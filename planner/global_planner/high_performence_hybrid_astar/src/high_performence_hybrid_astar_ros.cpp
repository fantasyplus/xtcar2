...
void HybridAstar::SetOccupancyGrid(const nav_msgs::OccupancyGrid &cost_map)
{
    ...
    // ROS_INFO("HybridAstar:costmap height:%d,width:%d,resolution:%f", _cost_map.info.height, _cost_map.info.width, _cost_map.info.resolution);
    ...
    _is_obstacle_table.resize(map_width);
    for (int i = 0; i < map_width; i++)
    {
        _is_obstacle_table[i].resize(map_height);
        for (int j = 0; j < map_height; j++)
        {
            const auto cost = _cost_map.data[j * map_width + i];
    ...
    {
        //生成维诺图，用于之后的轨迹平滑
        bool **bin_map = new bool *[map_width];
    ...
    ROS_INFO("HybridAstar:HybridAstar:SetOccupancyGrid successfully,cost time :%f ms", t0.getTimerMilliSec());
}

std::vector<std::vector<NodeUpdate>> HybridAstar::createTransitionTable(const float minimum_turning_radius,
                                                                        const float maximum_turning_radius,
                                                                        ...
                                                                        const bool use_back)
{
    ...
    const float dtheta = 2.0 * M_PI / static_cast<float>(theta_size);
    ...
    //  const float step_min = R_min * dtheta;
    float step_min = std::sqrt(2) * _cost_map.info.resolution; //单次步长一定会走出一个格子
    step_min = (R_min * dtheta) < step_min ? R_min * dtheta : step_min;

    float dR = 0.0;
    if (turning_radius_size != 0)
        dR = (R_max - R_min) / static_cast<float>(turning_radius_size);
    ...
    {
        float R = R_min + i * dR; // R从最小增加到最大

        // float step = R * dtheta;  //单次路径长度为曲线的弧长（也就是gcost）
        //和最短步长比较
        float step = (R * dtheta) > step_min ? R * dtheta : step_min;
    ...
    ROS_INFO("DEBUG:forward_node_candidates.size():%d", (int)forward_node_candidates.size());
...
SearchStatus HybridAstar::makePlan(const geometry_msgs::Pose &start_pose, const geometry_msgs::Pose &goal_pose)
{
    ...
    // ROS_INFO("HybridAstar:get start pose:%f,%f", _start_pose.position.x, _start_pose.position.y);
    // ROS_INFO("HybridAstar:get goal pose:%f,%f", _goal_pose.position.x, _goal_pose.position.y);
...
SearchStatus HybridAstar::search()
{
    ...
    ROS_INFO("HybridAstar:----------start search----------");
    ...
    {
        //当前时间减去初始时间
        const float msec = t0.getTimerMilliSec();
        //超时
        ...
        {
            // ROS_INFO("HybridAstar:current_node has closed");
            _open_list.pop();
            continue;
        }
        ...
        if (_planner_common_param.use_analytic_expansion)
        {
            ...
            {
                ROS_INFO("HybridAstar:analytic expansion successfully");
        ...
        if (isGoal(*current_node))
        {
            ...
            ROS_INFO("HybridAstar:research_cnt:%d", research_cnt);
        ...
        for (const auto &transition : _transition_table[index_theta])
        {
            ...
            */
            ...
            const float next_gc = current_node->g_cost + move_cost;
...
bool HybridAstar::setStartNode()
{
    ...
    start_node->theta = 2.0 * M_PI / static_cast<float>(_planner_common_param.theta_size) *
                        static_cast<float>(start_index.theta_index);
    start_node->g_cost = 0;
...
void HybridAstar::setPath(AstarNodePtr goal_node)
{
    ...
    ROS_INFO("HybridAstar:path_length:%f", path_length);
...
float HybridAstar::calcDistance2d(const geometry_msgs::Point &p1, const geometry_msgs::Point &p2)
{
    return std::hypot(p2.x - p1.x, p2.y - p1.y);
}

float HybridAstar::calcDistance2d(const geometry_msgs::Pose &p1, const geometry_msgs::Pose &p2)
{
    float theta_cost = 0;
...
float HybridAstar::getDistanceHeuristic(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal)
{
...
float HybridAstar::getObstacleHeuristic(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal)
{
    // ROS_INFO("HybridAstar:start 2d A*");
    ...
    {
        research_cnt++;
        // ROS_INFO("HybridAstar:_2d_memory_open_nodes size():%d", _2d_memory_open_nodes.size());
        // ROS_INFO("HybridAstar:_2d_open_list size():%d", _2d_open_list.size());
    ...
    while (!_2d_open_list.empty())
    {
        ...
        {
            // ROS_INFO("HybridAstar:find 2d A* goal:%d,%d", goal_2d->x_index, goal_2d->y_index);
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            float move_cost = sqrt(pow(dx[i], 2.0) + pow(dy[i], 2.0));
            if (_planner_common_param.use_theta_cost)
            {
            ...
            const float next_gc = current_node_2d->g_cost + move_cost;
    ...
    // ROS_INFO("HybridAstar:can't find 2d a* goal");
    return 10000;
}

float HybridAstar::estimateCost(const geometry_msgs::Pose &start)
{
...
void HybridAstar::computeCollisionIndexes(const int theta_index, std::vector<IndexXY> &index_2d)
{
    ...
    const float back = -1.0 * vehicle_shape.cg2back;
    const float front = vehicle_shape.length - vehicle_shape.cg2back;
    const float right = -1.0 * vehicle_shape.width / 2;
    const float left = vehicle_shape.width / 2;
    ...
    const float base_theta = tf2::getYaw(base_pose.orientation);

    const float _cost_mapresolution = static_cast<float>(_cost_map.info.resolution);

    //用一个一维数组保存车体大小范围内的格点索引
    for (float x = back; x <= front; x += _cost_mapresolution)
    {
        for (float y = right; y <= left; y += _cost_mapresolution)
        {
            //直角坐标系下逆时针的旋转公式
            //计算不同转向角下的车体坐标（如果base_theta为0，也就不变）
            const float offset_x = x * std::cos(base_theta) - y * std::sin(base_theta);
            const float offset_y = x * std::sin(base_theta) + y * std::cos(base_theta);
...
void HybridAstar::visualOpenNode()
{
    int clear_index = 0;
    static int id = 0;
    ROS_INFO("HybridAstar:_memory_open_nodes size():%d", (int)_memory_open_nodes.size());
...
inline bool HybridAstar::isOutOfRange(const IndexXYT &index) const
{
    ...
    {
        // ROS_INFO("HybridAstar:HybridAstar:out of range,x_index:%d,y_index:%d", index.x_index, index.y_index);
        return true;
    }
...
inline bool HybridAstar::isObstacle(const IndexXYT &index) const
{
    ...
    if (_is_obstacle_table[index.x_index][index.y_index])
    {
        // ROS_INFO("HybridAstar:HybridAstar:is obstacle,x_index:%d,y_index:%d", index.x_index, index.y_index);
        return true;
    }
    else
    {
...
float HybridAstar::normalizeRadian(const float rad, const float min_rad)
{
    float radian = wrap_angle(rad);
    return radian < min_rad ? radian += 2.0 * M_PI : radian;
}

int HybridAstar::getThetaIndex(const float theta, const int theta_size)
{
    //当前角度除以单位角，再对离散角的总个数取余，就可以获得当前角在2π下的索引
    const float angle_increment_rad = 2.0 * M_PI / static_cast<float>(theta_size);
...
{
    const auto resolution = static_cast<float>(_cost_mapresolution);
...
geometry_msgs::Pose HybridAstar::index2pose(const IndexXYT &index, const float &_cost_mapresolution, const int theta_size)
{
    ...
    const float angle_increment_rad = 2.0 * M_PI / static_cast<float>(theta_size);
    const float yaw = index.theta_index * angle_increment_rad;
    tf2::Quaternion quat;
    quat.setRPY(0, 0, yaw);
...
constexpr float HybridAstar::deg2rad(const float deg)
{
    return deg * M_PI / 180.0;
}

inline geometry_msgs::Quaternion HybridAstar::getQuaternion(const float yaw)
{
    tf2::Quaternion q;
...
void HybridAstar::smoothPath(TrajectoryWaypoints &initial_traj)
{
    ...
    //最大迭代次数
    const int max_interations = 1500;
    int iteration = 0;
                                               ...
                                               float &closest_distance)
{
    ...
    if (analytic_iterations <= 0)
    {
        ...
        {
            //开始插值
            rs_state.interpolateByXt(from(), to(), (float)i / num_intervals, s(), is_back);
            reals = s.reals();
            ...
            if (detectCollision(index_2_collision))
            {
                ...
                rs_state.clearStaticIndexPrev();
                // ROS_INFO("HybridAstar:-------collision-------");
                return nullptr;
            }
            else
            {
        ...
        {
            // ROS_INFO("HybridAstar:node back status is: %d", node->is_back);
            node->parent = prev;
        ...
        // ROS_INFO("HybridAstar:successful analytic node is %d", (int)analytic_candidate_nodes.size());
        return prev;
    }
