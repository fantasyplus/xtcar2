...
public:
    ...
    void setNodeHandle(const std::string name)
    {
        ros::NodeHandle _nh("~/" + name);
...
private:
    ...
    const float kDoublePi = 2.0 * M_PI;
    template <typename T>
    ...
    float normalizeRadian(const float rad, const float min_rad = -M_PI);

    //获取当前角度的索引值
    int getThetaIndex(const float theta, const int theta_size);
    ...
    //计算欧几里得距离
    float calcDistance2d(const geometry_msgs::Point &p1, const geometry_msgs::Point &p2);
    //计算欧几里得距离
    float calcDistance2d(const geometry_msgs::Pose &p1, const geometry_msgs::Pose &p2);

    //将yaw角转化为quaternion四元数
    inline geometry_msgs::Quaternion getQuaternion(const float yaw);
...
private:
    ...
    //创建节点扩张所需的状态转移表
    std::vector<std::vector<NodeUpdate>> createTransitionTable(const float minimum_turning_radius,
    ...
    //计算启发式值，即h值
    float estimateCost(const geometry_msgs::Pose &start);

    //计算距离启发值
    float getDistanceHeuristic(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal);

    //计算障碍物启发值
    float getObstacleHeuristic(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal);
